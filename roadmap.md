...
- [ ] Streaming / large-result handling (cursor or fetch-more pagination instead of LIMIT/OFFSET)
  - [ ] Define a driver-neutral row-stream contract with explicit close/cancellation semantics and focused unit tests
  - [ ] Implement filtered and unfiltered table row streams in the PostgreSQL, SQLite, and MySQL drivers, preserving each driver's existing ordering and parameterized filters
