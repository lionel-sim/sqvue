...
- Target Go 1.25; run `gofmt` and `golangci-lint run` on changed Go code, then `go test ./...` before completing code changes. If the environment blocks a check, report the limitation and run the narrowest relevant checks that work.
...
- Add focused tests for new behavior and run formatting, `golangci-lint run`, relevant narrow tests, then `go test ./...`; use race tests and `go vet ./...` for asynchronous or stateful TUI work.
