...
func (d *Driver) DeleteRow(ctx context.Context, request db.RowDeleteRequest) error {
	...
	query, args, err := sqliteDeleteRowStatement(request)
	if err != nil {
		return err
	}
	result, err := d.db.ExecContext(ctx, query, args...)
...
func sqliteDeleteRowStatement(request db.RowDeleteRequest) (string, []any, error) {
	if err := request.Validate(); err != nil {
		return "", nil, err
	}
	where := make([]string, 0, len(request.PrimaryKey))
...
func (d *Driver) columnMetadata(ctx context.Context, schema, table string) ([]db.Column, error) {
	...
	query := fmt.Sprintf("pragma %s.table_xinfo(%s)", quoteIdent(schema), quoteIdent(table))
	...
	for rows.Next() {
		var ordinal, notNull, primaryKey, hidden int
		var name, dataType string
		var defaultValue sql.NullString
		if err := rows.Scan(&ordinal, &name, &dataType, &notNull, &defaultValue, &primaryKey, &hidden); err != nil {
			return nil, err
		}
		column := db.Column{
			Name:            name,
			...
			IsAutoGenerated: primaryKey > 0 && strings.EqualFold(strings.TrimSpace(dataType), "integer"),
			IsGenerated:     hidden != 0,
		}
