...
func TestBrowseWhereUsesParameterizedOperators(t *testing.T) {
	...
	wantWhere := " where `name` = ? and lower(cast(`note` as char)) like lower(?) escape '\\\\' and cast(`code` as char) like ? and `rank` > ? and `rank` < ? and `deleted_at` is null and `email` is not null"
	if where != wantWhere {
