...
func mysqlInsertRowStatement(request db.RowInsertRequest) (string, []any, error) {
	if err := request.Validate(); err != nil {
		return "", nil, err
	}
	qualifiedTable := qualifiedName(request.Table.Schema, request.Table.Name)
	if len(request.Values) == 0 {
		return "insert into " + qualifiedTable + " () values ()", nil, nil
	}
...
func (d *Driver) columnMetadata(ctx context.Context, schema, table string) ([]db.Column, error) {
	...
	rows, err := d.db.QueryContext(ctx, `
		select column_name, column_type, is_nullable, column_default, column_key = 'PRI',
		       extra like '%auto_increment%' as is_auto_generated,
		       coalesce(generation_expression, '') <> '' as is_generated
	...
	for rows.Next() {
		...
		var primary, auto, generated bool
		if err := rows.Scan(&name, &dataType, &nullable, &defaultValue, &primary, &auto, &generated); err != nil {
			return nil, err
		}
		column := db.Column{Name: name, DataType: dataType, Nullable: nullable == "YES", IsPrimary: primary, IsAutoGenerated: auto, IsGenerated: generated}
