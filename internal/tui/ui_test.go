...
func TestSQLHistoryNavigationRestoresDraftAndStaysPerProfile(t *testing.T) {
	...
	m, _ = m.handleSQLHistoryKey(tea.KeyMsg{Type: tea.KeyCtrlP})
...
func TestRowInsertUsesExplicitFieldStates(t *testing.T) {
	client := &fakeDriver{}
	m := New(Options{Client: client, Timeout: time.Second})
	m.tables = []db.Table{{Schema: "public", Name: "items", Type: "table"}}
	m.focused = true
	m.columns = []db.Column{
		{Name: "id", DataType: "integer", IsPrimary: true, IsAutoGenerated: true},
		{Name: "name", DataType: "text"},
		{Name: "note", DataType: "text", Nullable: true},
		{Name: "created_at", DataType: "timestamp", Default: stringPtr("CURRENT_TIMESTAMP")},
	}

	m, _ = update(m, keyMsg("a"))
	if m.activeOverlay != overlayRowInsert || !m.helpKeyMap().AddRow.Enabled() {
		t.Fatalf("insert form = overlay %v, add row enabled %t", m.activeOverlay, m.helpKeyMap().AddRow.Enabled())
	}
	m.rowInsertCursor = 1
	m, _ = update(m, tea.KeyMsg{Type: tea.KeyEnter})
	m.rowInsertInput.SetValue("NULL")
	m, _ = update(m, tea.KeyMsg{Type: tea.KeyEnter})
	if got := m.rowInsertFields[1]; got.mode != rowInsertLiteral || got.value != "NULL" {
		t.Fatalf("literal NULL field = %#v", got)
	}
	m.rowInsertCursor = 2
	...
	if len(request.Values) != 3 || request.Values[0] != (db.RowInsertValue{Column: "name", Kind: db.RowInsertLiteral, Value: "NULL"}) || request.Values[1] != (db.RowInsertValue{Column: "note", Kind: db.RowInsertNull}) || request.Values[2] != (db.RowInsertValue{Column: "created_at", Kind: db.RowInsertCurrentTimestamp}) {
		t.Fatalf("row insert request = %#v", request)
	}
}

func TestRowInsertRequiresValueAndRestrictsNowToDateTime(t *testing.T) {
	m := New(Options{Client: &fakeDriver{}, Timeout: time.Second})
	m.tables = []db.Table{{Schema: "public", Name: "items", Type: "table"}}
	m.focused = true
	m.columns = []db.Column{{Name: "name", DataType: "text"}}
	m, _ = update(m, keyMsg("a"))
	m, _ = update(m, keyMsg("t"))
	if !strings.Contains(m.status, "not a date/time column") {
		t.Fatalf("NOW status = %q", m.status)
	}
	if _, err := m.rowInsertRequest(); err == nil || !strings.Contains(err.Error(), "name requires a value") {
		t.Fatalf("required insert error = %v", err)
	}
}

func TestRowInsertIsDisabledForViewsAndSQLResults(t *testing.T) {
	m := New(Options{Client: &fakeDriver{}, Timeout: time.Second})
	m.tables = []db.Table{{Schema: "public", Name: "item_names", Type: "view"}}
	m.focused = true
	m.columns = []db.Column{{Name: "name"}}
	if m.helpKeyMap().AddRow.Enabled() {
		t.Fatal("add row is enabled for a view")
	}
	m, _ = update(m, keyMsg("a"))
	if m.status != "inserting is available only for tables" {
		t.Fatalf("view insert status = %q", m.status)
	}

	m.tables[0].Type = "table"
	m.queryActive = true
	if m.helpKeyMap().AddRow.Enabled() {
		t.Fatal("add row is enabled for SQL results")
	}
	m, _ = update(m, keyMsg("a"))
	if m.status != "inserting is unavailable for SQL results" {
		t.Fatalf("SQL insert status = %q", m.status)
	}
}

func TestConfirmedRowInsertRefreshesAndKeepsBrowseState(t *testing.T) {
	client := &fakeDriver{cols: []db.Column{{Name: "id", IsPrimary: true, IsAutoGenerated: true}, {Name: "name"}}, rows: [][]string{{"3", "created"}}}
	m := New(Options{Client: client, Timeout: time.Second})
	m.tables = []db.Table{{Schema: "public", Name: "items", Type: "table"}}
	m.focused, m.page, m.pageSize = true, 1, 2
	m.columns = append([]db.Column(nil), client.cols...)
	m.rows, m.visibleColumns = [][]string{{"3", "before"}}, []bool{true, true}
	m.browseFilters = []db.RowFilter{{Column: "name", Operator: db.FilterContains, Value: "created"}}
	m.browseSort = db.SortSpec{Column: "name"}

	m, _ = update(m, keyMsg("a"))
	m.rowInsertCursor = 1
	m, _ = update(m, tea.KeyMsg{Type: tea.KeyEnter})
	m.rowInsertInput.SetValue("created")
	m, _ = update(m, tea.KeyMsg{Type: tea.KeyEnter})
	m, _ = update(m, tea.KeyMsg{Type: tea.KeyCtrlR})
	if m.activeOverlay != overlayRowInsertConfirm {
		t.Fatalf("confirmation overlay = %v", m.activeOverlay)
	}
	m, cmd := update(m, tea.KeyMsg{Type: tea.KeyEnter})
	m, cmd = update(m, runCmd(cmd))
	if got := client.lastRowInsert; got.Table.Name != "items" || len(got.Values) != 1 || got.Values[0] != (db.RowInsertValue{Column: "name", Kind: db.RowInsertLiteral, Value: "created"}) {
		t.Fatalf("row insert request = %#v", got)
	}
	m, _ = update(m, runCmd(cmd))
	if m.status != "inserted row; active filters may hide it" || !m.focused || m.page != 1 || m.browseSort != (db.SortSpec{Column: "name"}) || len(m.browseFilters) != 1 || len(m.visibleColumns) != 2 {
		t.Fatalf("refreshed insert state = %#v", m)
	}
}

func TestRowInsertFailureKeepsFormValues(t *testing.T) {
	client := &fakeDriver{rowInsertErr: errBoom}
	m := New(Options{Client: client, Timeout: time.Second})
	m.tables = []db.Table{{Schema: "public", Name: "items", Type: "table"}}
	m.focused = true
	m.columns = []db.Column{{Name: "name"}}
	...
	m, cmd := update(m, tea.KeyMsg{Type: tea.KeyEnter})
	m, _ = update(m, runCmd(cmd))
	if m.activeOverlay != overlayRowInsert || m.lastErr != errBoom || m.rowInsertFields[0].value != "created" {
		t.Fatalf("insert failure state = overlay %v, error %v, fields %#v", m.activeOverlay, m.lastErr, m.rowInsertFields)
	}
}
